...
} = require("../enums/supportedMultifileSetupTypes.js");
      ...
      memoryCheckInterval = setInterval(async () => {
        ...
        if (
          ...
          if (childProcess) {
            ...
            logger.warn(
              "Child process is undefined and response is on way, trying to send another response"
            );
...
const _prepareErrorMessage = (outputLog, language, command) => {
  ...
  if (
    errorMsg.substring(errorMsg.length - subString.length, errorMsg.length) ===
      subString ||
  ...
  evaluatedResponses.forEach((res) => {
    if (res.status === "fulfilled") {
      ...
      if (validatedData.error || openAIResponse.points !== points) {
        logger.error(
          `The response received from Open AI failed the validation check: ${JSON.stringify(
            validatedData
          )}`
        );
        ++errorResponsesCount;
...
const _executeCode = async (req, res, response) => {
    ...
    try {
      ...
      if (langConfig.compile) {
        compileCommand = `cd /tmp/ && ${langConfig.compile}`;
        // Run compile command
        const compileLog = await _runScript(compileCommand, res, true);
        response.compileMessage =
          compileLog.error !== undefined
            ? _prepareErrorMessage(compileLog, language, compileCommand)
            : "";
      } else {
        // For languages that don't require compilation (e.g., Bash), set compileMessage to empty
        response.compileMessage = "";
      }
...
const _getAiScore = async (
  ...
  try {
    ...
    let { allValidResponses, errorResponsesCount } = await _executePrompt(
      3,
      langConfig,
      prompt,
      points
    ...
    if (scoreConfidence.frequency !== 3) {
      ...
      if (
        scoreConfidence.frequency / scoreConfidence.total < 0.5 &&
        totalValidRequests < 10
      ) {
        const {
          allValidResponses: additionalValidResponses,
    ...
    while (totalRequests < 20) {
      const { allValidResponses: additionalValidResponses } =
    ...
    if (allValidResponses.length < 10) {
      throw new Error(
        "We were not able to achieve 10 valid evaluations from Open AI to generate a confidence"
      );
    }
...
const _executeSqlQueries = async (dbPath, queries) => {
  ...
  for (let i = 0; i < sqlStatements.length; i++) {
    try {
      ...
      return {
        error: true,
        data: `${err.message} at statement ${i + 1}`,
      };
    }
  }
};
...
const _startStaticServer = async (rootPath) => {
  ...
  return new Promise((resolve, reject) => {
    staticServerInstance
      .listen(appConfig.multifile.jasminePort, () => {
...
const _startJasmineServer = async () => {
  return new Promise((resolve, reject) => {
    const jasmineServer = spawn("npm", ["run", "test:serve"], {
      cwd: appConfig.multifile.workingDir,
      detached: true,
    }); // run independent of parent to prevent it from getting orphan
    let isRejected = false;
...
const _runTests = async () => {
  ...
  try {
    ...
    page.on("requestfailed", (request) => {
      logger.error(
        `Request to ${request.url()} failed with reason ${request.abortErrorReason()}`
      );
    });
    page.on("response", (response) => {
      if (!response.ok()) {
        logger.error(
          `Failed response: ${response.url()} - ${response.status()} ${response.statusText()}`
        );
      }
    });
    ...
    const resp = await page.goto(
      `http://localhost:${appConfig.multifile.jasminePort}/`
    );
    if (resp.status() !== 200) {
    ...
    await page.waitForFunction(() =>
      document
        .querySelector(".jasmine-duration")
    ...
    jasmineResults = await page.evaluate((summaryElement) => {
      ...
      suiteElements.forEach((suiteElement) => {
        const specElements = suiteElement.querySelectorAll(
          ":scope > .jasmine-specs"
        ); // only look at direct children to avoid duplicates

        specElements.forEach((specElement) => {
          const passedTests = Array.from(
            specElement.querySelectorAll(".jasmine-passed"),
            (el) => el.textContent
          );
          const failedTests = Array.from(
            specElement.querySelectorAll(".jasmine-failed"),
            (el) => el.textContent
          );
  ...
  return new Promise((resolve, reject) => {
    ...
    lsof.on("close", (code) => {
      ...
      if (code === 1 && stderr.trim() === "") {
        // lsof returns 1 when the port is idle
        ...
        logger.info(
          `Port ${port} is occupied. Attempting to kill the process...`
        );
...
const _executeMultiFile = async (req, res, response) => {
  ...
  try {
    await _preCleanUp();
    const fileContent = await _getSubmissionDataFromGCS(
      req.url,
      appConfig.multifile.submissionFileDownloadPath
    );
    await _writeFilesToDisk(fileContent, appConfig.multifile.workingDir);
  } catch (err) {
    logger.error(err);
    throw new Error(
      "Error in running multifile submission, check service logs for the issue"
    );
  }
  ...
  try {
    ...
    if (req?.non_editable_files) {
      ...
      if (!isValidSubmission)
        throw new Error(`A non editable file has been modified, exiting...`);
    }
    if (req.type === FRONTEND_STATIC_JASMINE) {
      const staticServerInstance = await _startStaticServer(
        appConfig.multifile.staticServerPath
      );
    ...
    await _cleanUpDir(
      appConfig.multifile.workingDir,
      appConfig.multifile.submissionFileDownloadPath
    );
    response.output = jasmineResults;
  } catch (err) {
    if (
      err.message === "No package.json found" ||
      err.message.includes("Browser was not found at")
    ) {
      throw err;
    } else {
